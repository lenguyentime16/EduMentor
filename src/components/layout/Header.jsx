  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (dropdownRef.current && !dropdownRef.current.contains(event.target)) {
        // Kiểm tra xem có phải click vào logout button hoặc menu items không
        const isLogoutButton = event.target.closest('[data-logout-button]');
        const isMenuItem = event.target.closest('[data-menu-item]');
        if (!isLogoutButton && !isMenuItem) {
          setIsUserDropdownOpen(false);
        }
      }
    };
        ...
        <div className="absolute right-0 top-full mt-2 w-72 bg-white rounded-lg shadow-lg border border-gray-200 py-2 z-50 animate-in slide-in-from-top-2 duration-200">
          ...
          <div className="py-2">
            ...
            <Link
              ...
              className="w-full flex items-center space-x-3 px-4 py-3 hover:bg-gray-50 transition-colors duration-150 text-decoration-none"
              data-menu-item="true"
            >
